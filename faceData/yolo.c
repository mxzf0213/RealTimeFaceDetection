...
{
    char *train_images = "/content/faceData/train.txt";
    char *backup_directory = "/content/faceData/backup/";
    srand(time(0));
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        if(i%500==0 || i<10)printf("Loaded: %lf seconds\n", sec(clock()-time));
        ...
        if(i%500==0 || i<10)printf("%d: %f, %f avg, %f rate, %lf seconds, %d images\n", i, loss, avg_loss, get_current_rate(net), sec(clock()-time), i*imgs);
        if(i%500 == 0){
            char buff[256];
